import { useDoctor, usePatient } from "../../Context";
...
const GenderPopUp = ({ settingsPup, setSettingsPup }) => {
  ...
  const [gender, setGender] = useState(doctorDetail.gender);

  const onSubmit = async () => {
    setSubmit(true);
    if (doctorDetail.gender === gender) return;
    try {
      console.log(doctorDetail);
      updateDB("doctors", auth.currentUser.uid, { gender: gender });
      setSubmit(false);
      await getDoctorData();
      setSettingsPup("");
    } catch (err) {
  ...
  return (
    <div>
      <div
        className={` ${
          settingsPup === "Gender" ? "bottom-0 top-0" : "top-[200%]"
        } fixed  p-4 h-screen  bg-brandwhite z-[55] duration-200 w-full right-0 `}
      >
        <div>
          <Header
            text={doctorDetail.gender ? "Edit your gender" : "Add your gender"}
            ...
            <div className=" flex flex-col relative justify-end  w-full text-[14px]">
              ...
              <div className=" mt-4 flex-1 bg-gray-100  rounded-lg grid grid-cols-2 p-[2px] text-base ">
                <button
                  ...
                  className={`${
                    gender === "Male" ? " bg-white  text-[12p.5x]" : " "
                ...
                <button
                  ...
                  className={`${
                    gender === "Female" ? " bg-white  text-[12p.5x]" : " "
        ...
        <div
          className={` ${
            settingsPup === "Gender" ? "bottom-0" : "bottom-[-200%]"
        ...
        >
          ...
          <button
            disable={submit && doctorDetail.gender === gender ? true : false}
            type="submit"
            onClick={onSubmit}
            className={` ${
              doctorDetail.gender === gender ? "bg-gray-500" : "bg-primary"
